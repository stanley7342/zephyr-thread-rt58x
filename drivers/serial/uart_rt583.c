/*
 * Zephyr UART driver for Rafael RT583
 *
 * RULES (learned the hard way):
 * 1. Never call printk from this driver — ISR context causes spinlock deadlock.
 * 2. uart_rt583_init must return 0 (not hosal_uart_init return value).
...
#define DT_DRV_COMPAT rafael_rt583_uart
...
static int uart_rt583_poll_in(const struct device *dev, unsigned char *c)
{
    struct uart_rt583_data *data = dev->data;
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
...
static void uart_rt583_poll_out(const struct device *dev, unsigned char c)
{
    struct uart_rt583_data *data = dev->data;
...
{
    struct uart_rt583_data *data =
        CONTAINER_OF(work, struct uart_rt583_data, tx_work);
    if (data->tx_irq_enabled && data->irq_cb)
        data->irq_cb(data->self, data->irq_cb_data);
}

static int uart_rt583_fifo_fill(const struct device *dev,
...
static int uart_rt583_fifo_read(const struct device *dev,
...
static void uart_rt583_irq_tx_enable(const struct device *dev)
{
    struct uart_rt583_data *data = dev->data;
...
static void uart_rt583_irq_tx_disable(const struct device *dev)
{
    struct uart_rt583_data *data = dev->data;
    data->tx_irq_enabled = false;
}

static int uart_rt583_irq_tx_ready(const struct device *dev)
{
    struct uart_rt583_data *data = dev->data;
    return data->tx_irq_enabled ? 1 : 0;
}

static int uart_rt583_irq_tx_complete(const struct device *dev)
{
    (void)dev; return 1;
}

static void uart_rt583_irq_rx_enable(const struct device *dev)
{
    struct uart_rt583_data *data = dev->data;
...
static void uart_rt583_irq_rx_disable(const struct device *dev)
{
    struct uart_rt583_data *data = dev->data;
    if (data->rx_irq_enabled) {
        hosal_uart_callback_set(&data->hosal_dev, HOSAL_UART_RX_CALLBACK,
...
static int uart_rt583_irq_rx_ready(const struct device *dev)
{
    struct uart_rt583_data *data = dev->data;
    return rx_ring_empty(&data->rx_ring) ? 0 : 1;
}

static int uart_rt583_irq_is_pending(const struct device *dev)
{
    return uart_rt583_irq_rx_ready(dev);
}

static int uart_rt583_irq_update(const struct device *dev)
{
    (void)dev; return 1;
}

static void uart_rt583_irq_callback_set(const struct device *dev,
...
static const struct uart_driver_api uart_rt583_api = {
    .poll_in  = uart_rt583_poll_in,
    .poll_out = uart_rt583_poll_out,
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
    .fifo_fill        = uart_rt583_fifo_fill,
    .fifo_read        = uart_rt583_fifo_read,
    .irq_tx_enable    = uart_rt583_irq_tx_enable,
    .irq_tx_disable   = uart_rt583_irq_tx_disable,
    .irq_tx_ready     = uart_rt583_irq_tx_ready,
    .irq_tx_complete  = uart_rt583_irq_tx_complete,
    .irq_rx_enable    = uart_rt583_irq_rx_enable,
    .irq_rx_disable   = uart_rt583_irq_rx_disable,
    .irq_rx_ready     = uart_rt583_irq_rx_ready,
    .irq_is_pending   = uart_rt583_irq_is_pending,
    .irq_update       = uart_rt583_irq_update,
    .irq_callback_set = uart_rt583_irq_callback_set,
#endif
};
...
static void uart_rt583_isr(const void *arg)
{
    const struct device *dev = arg;
    const struct uart_rt583_cfg *cfg = dev->config;
    switch (cfg->uart_id) {
...
#define UART_RT583_DEVICE(inst)                                              \
    static struct uart_rt583_data uart_rt583_data_##inst;                    \
                                                                             \
    static const struct uart_rt583_cfg uart_rt583_cfg_##inst = {             \
        ...
        .rx_pin    = DT_INST_PROP_OR(inst, rx_pin, 255),                     \
                                                                             ...
                                                                             \
    static int uart_rt583_init_##inst(const struct device *dev)              \
    {                                                                        \
        const struct uart_rt583_cfg *cfg  = dev->config;                     \
        struct uart_rt583_data      *data = dev->data;                       \
                                                                             \
    ...
    DEVICE_DT_INST_DEFINE(inst,                                              \
                          uart_rt583_init_##inst,                            \
                          NULL,                                              \
                          &uart_rt583_data_##inst,                           \
                          &uart_rt583_cfg_##inst,                            \
                          PRE_KERNEL_1,                                      \
                          CONFIG_SERIAL_INIT_PRIORITY,                       \
                          &uart_rt583_api);

DT_INST_FOREACH_STATUS_OKAY(UART_RT583_DEVICE)
