#!/usr/bin/env bash
# RT583-EVB Zephyr + OpenThread — macOS build script
#
# Usage:
#   bash scripts/macos/build.sh -p thread                  # Thread FTD CLI (clean)
#   bash scripts/macos/build.sh -p thread -i               # Thread FTD CLI (incremental)
#   bash scripts/macos/build.sh -p bootloader              # MCUboot (clean)
#   bash scripts/macos/build.sh -p bootloader -i           # MCUboot (incremental)
#
# Output:
