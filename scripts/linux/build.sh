#!/usr/bin/env bash
# RT583-EVB Zephyr + OpenThread — Linux/WSL 編譯腳本
#
# 用法：
#   bash scripts/linux/build.sh -p thread                  # Thread FTD CLI（Clean）
#   bash scripts/linux/build.sh -p thread -i               # Thread FTD CLI（增量）
#   bash scripts/linux/build.sh -p bootloader              # MCUboot（Clean）
#   bash scripts/linux/build.sh -p bootloader -i           # MCUboot（增量）
#
# 輸出：
