 ...
 *
 * Original: Rafael-IoT-SDK-Internal/components/platform/hosal/rt582_hosal/Src/hosal_rf.c
 *
    ...
    if (rxCmdError == RF_MCU_RX_CMDQ_GET_SUCCESS) {
        ...
        case HOSAL_RF_RUCI_CMN_EVENT:
            ...
            if (k_msgq_put(&g_rf_evt_msgq, (void *)&evt_ptr, K_NO_WAIT) != 0) {
                /* Queue full — discard and free to avoid leak */
...
uint32_t hosal_rf_read_event(uint8_t *event_data_ptr) {
    ...
    if (k_msgq_get(&g_rf_evt_msgq, (void *)&pdata, K_FOREVER) != 0) {
