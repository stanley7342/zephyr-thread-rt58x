...
void RfMcu_HostBusySetAhb(bool status)
{
    RF_MCU_AHB_INTR_EN_PTR->HOST_BUSY = (status) ? 1 : 0;
}
...
{
#if (RF_MCU_USING_REG_FIELD)
    while (RF_MCU_AHB_INTR_STATUS_PTR->DMA_IS_BUSY != 0);
#else
...
bool RfMcu_RxQueueIsReadyAhb(void)
{
    ...
    {
#define RF_MCU_BMU_STA1_REG_ADDR              (0x004C)
#define RF_MCU_BMU_STA1_RXQ_EMPTY_BIT         (BIT17)
        uint8_t rx_data[4] = {0};
        uint32_t reg_value;

        RfMcu_MemoryGetAhb(RF_MCU_BMU_STA1_REG_ADDR, rx_data, 4) ;
        reg_value = (rx_data[0]) | (rx_data[1] << 8) | (rx_data[2] << 16) | (rx_data[3] << 24);

        if (reg_value & RF_MCU_BMU_STA1_RXQ_EMPTY_BIT)
        {
            return FALSE;
        }
        return TRUE;
...
#if (RF_MCU_USING_REG_FIELD)
    data_length = (RF_MCU_RX_INFO_PTR->RX_PKT_LEN & COMM_SUBSYS_MAX_RX_Q_LEN); // dummy read, workaround for HW bug
