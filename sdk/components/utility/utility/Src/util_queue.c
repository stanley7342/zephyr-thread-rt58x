...
queue_elem_t* queue_peek(const queue_t* p_queue) { return p_queue->p_front; }
...
void queue_iterator_elem_remove(queue_elem_iterator_t* p_it) {
    ...
    if (*p_it->pp_elem == p_it->p_queue->p_back) {
        if (p_it->pp_elem == &p_it->p_queue->p_front) {
            ...
            p_it->p_queue->p_back = PARENT_BY_FIELD_GET(queue_elem_t, p_next,
                                                        p_it->pp_elem);
        }
    }
...
void queue_iterator_elem_insert(queue_elem_iterator_t* p_it,
                                queue_elem_t* p_elem) {
