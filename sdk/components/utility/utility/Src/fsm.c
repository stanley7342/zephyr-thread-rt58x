...
static bool fsm_event_post_try(fsm_t* p_fsm, fsm_event_id_t event_id,
                               void* p_data, uint8_t start_table_idx);
...
static bool fsm_event_post_try(fsm_t* p_fsm, fsm_event_id_t event_id,
    ...
    p_transition_table_end = p_transition_table
                             + p_fsm_const->transitions_count;

    if (start_table_idx == 0 && p_fsm->any_state_transitions_index != 0) {
        // look for the beginning of the current state's transitions
        for (p_transition = p_transition_table;
             p_transition < p_transition_table_end; p_transition++) {
    ...
    for (p_transition++; p_transition < p_transition_table_end;
         p_transition++) {
        ...
        if (p_transition->event_id == event_id) {
            ...
            if (fsm_transition_perform_try(p_fsm, p_transition, p_data))
                return true; // only the first satisfying transition is performed.
...
void fsm_event_post(fsm_t* p_fsm, fsm_event_id_t event_id, void* p_data) {
    ...
    if (!fsm_event_post_try(p_fsm, event_id, p_data, 0)) {
        if (p_fsm->any_state_transitions_index != FSM_INVALID_INDEX) {
            (void)fsm_event_post_try(p_fsm, event_id, p_data,
                                     p_fsm->any_state_transitions_index);
        }
    }
