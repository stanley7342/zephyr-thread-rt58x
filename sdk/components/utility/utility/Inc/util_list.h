 ...
 ****************************************************************************************
 */
static inline struct utils_list_hdr*
utils_list_pick(const struct utils_list* const list) {
    return list->first;
}

static inline struct utils_list_hdr*
utils_list_pick_last(const struct utils_list* const list) {
    return list->last;
}

static inline struct utils_list_hdr*
utils_list_next(const struct utils_list_hdr* const list_hdr) {
...
static inline void __utils_dlist_add(utils_dlist_t* node, utils_dlist_t* prev,
                                     utils_dlist_t* next) {
    node->next = next;
    node->prev = prev;
...
static inline void utils_dlist_add_tail(utils_dlist_t* node,
                                        utils_dlist_t* queue) {
    __utils_dlist_add(node, queue->prev, queue);
}

static inline void utils_dlist_del(utils_dlist_t* node) //detach node from list
{
 ...
 */
#define UTILS_DLIST_HEAD_INIT(name)                                            \
    { &(name), &(name) }
...
static inline void utils_slist_add_tail(utils_slist_t* node,
                                        utils_slist_t* head) {
    while (head->next) {
        head = head->next;
    }
...
static inline void utils_slist_init(utils_slist_t* head) { head->next = 0; }
...
#define utils_slist_for_each_entry_safe(queue, tmp, node, type, member)        \
    for (node = utils_container_of((queue)->next, type, member),               \
        tmp = (queue)->next ? (queue)->next->next : NULL;                      \
         &node->member; node = utils_container_of(tmp, type, member),          \
 ...
 * @param[in]   name    the list to be initialized.
 */
#define UTILS_SLIST_HEAD_INIT(name)                                            \
    { 0 }
 ...
 * @param[in]   name    the list to be initialized.
 */
#define UTILS_SLIST_HEAD(name) utils_slist_t name = UTILS_SLIST_HEAD_INIT(name)
