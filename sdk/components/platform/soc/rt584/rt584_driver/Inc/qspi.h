...
#ifndef _RT584_QSPI_H_
#define _RT584_QSPI_H_
 ...
 * \brief           Define Qspi definitions, structures, and functions
 * @{
 */
...
#define   QSPI_STATUS_TRANSFER_COMPLETE         (1<<0)  /*!< SPI QSPI_STATUS_TRANSFER_COMPLETE.   */
#define   QSPI_STATUS_TRANSFER_SHORT            (1<<1)  /*!< QSPI_STATUS_TRANSFER_SHORT.   */
...
typedef enum {
    SPI0_ID = 0,        /*!< SPI identifies  0.   */
    SPI1_ID = 1,        /*!< SPI identifies  1.   */
    SPI_ID_MAX = 2,
} spi_id_t;

typedef enum {
    SPI_CS0 = 0,         /*!< SPI Chip Select 0.   */
    SPI_CS1 = 1,         /*!< SPI Chip Select 1.   */
    SPI_CS2 = 2,         /*!< SPI Chip Select 2.   */
    SPI_CS3 = 3,         /*!< SPI Chip Select 3.   */
} qspi_manual_csx_t;
 ...
 * \param           status    QSPI transfer status.
 ...
 *                  PRE_DELAY_ENABLE: Enable pre-delay SCLK, this delay SCLK will be inserted when CS is active the dealy pre_delay SCLK.
 *                                    If system does not require this option, please set this option 0 to disable pre_delay.
 *                                    For slave device, this setting will be ignored.
 *                  INTER_DELAY_ENABLE: Enable inter-transfer dealy SCLK, this delay SCLK will be inserted between two continuous SPI transfer.
 *                                    If system does not require this option, please set this option 0 to disable inter_delay.
 *                                    For slave device, this setting will be ignored.
 *                  POST_DELAY_ENABLE:  Enable post-delay SCLK, this delay SCLK will be inserted before CS change from active to non-active.
 *                                    If system does not require this option, please set this option 0 to disable inter_delay.
 *                                    For slave device, this setting will be ignored.
 *                  PRE_DELAY_COUNT:   If PRE_DELAY_ENABLE is enable (1), number of "PRE_DELAY_COUNT+1" spi SCLK will be inserted
 *                                   between CS becomes active and sending the first SPI data. Maximum delay counter is 256 SPI SCLK.
 *                  INTER_DELAY_COUNT:  If INTER_DELAY_ENABLE is enable (1), number of "INTER_DELAY_COUNT+1" spi SCLK will be inserted between
 *                                    two continuous SPI transfer.
 *                  POSE_DELAY_COUNT:   If POST_DELAY_ENABLE is enable (1), number of "POST_DELAY_ENABLE+1" spi SCLK will be inserted
 *                                    before CS becomes inactive to stop transfer.
...
typedef struct {
    ...
    uint8_t   pre_delay_enable:     1;      /*!< enable pre-delay before sending the first SPI bit signal  */
    uint8_t   inter_delay_enable:   1;      /*!< enable interval delay between two continuous transfer CS */
    uint8_t   post_delay_enable:    1;      /*!< enable post-dealy after sending the last SPI bit signal */

    uint8_t   reserved2:      5;

    uint8_t   pre_delay_count;              /*!< enable pre-delay counter */
    uint8_t   inter_delay_count;            /*!< enable interval delay  counter  */
    uint8_t   pose_delay_count;             /*!< enable pre-delay counter  */
    uint8_t   reserved3;
...
typedef struct {
    ...
    uint8_t    select_slave_device : 2;     /*!< For SPI master mode, controller can select 4 slave devices to send SPI signal*/
#endif
 ...
 * \retval          STATUS_SUCCESS          ---  This controller setting is successful
 * \details
 *                  When SPI(QSPI) connects multiple spi devices. SPI(QSPI) can use
 *                      chip_select to enable device between maximum 4 spi slave, and
 *                      each slave device has itself "select polarity" (CS active low or active high)
 *                      to start/stop the spi transfer.
 *                  Using this API to set each spi device "select polarity" independently.
 *                  Please call API after qspi_init and before any SPI(QSPI) transfer.
 *                      If there is only one spi device for SPI, then this option can be set
 *                      in qspi_init function by value SPI_CS_POL and SPI_CS.
 *                  Then each transfer will use this default setting.
 */
...
/**
 *  \brief          This function is used for to change SPI_CLK..
 *  \param[in]      qspi_id    Specifies the controller id.  0 for QSPI0/ 1 for QSPI1.
...
/*@}*/ /* end of RT584_DRIVER QSPI */

#ifdef __cplusplus
}
#endif

#endif /* end of _RT584_QSPI_H__ */
