...
/**
 * \file           uart.h
 * \brief          uart driver definition header file
 */
/*
...
/**
 * \defgroup        UART_DRV Uart drv
 * \ingroup         RT584_DRIVER
 * \brief           Define Uart drv definitions, structures, and functions
 * @{
 */


/**
 * \brief           Notice: This define is not hardware register bit setting, so don't be confused with
...
typedef enum {
    ...
    UART_CLK_SRC_RCO1M     = 2,                 /*!< EVEN PARITY  */
    UART_CLK_SRC_RCO32K    = 3,                 /*!< ODD PARITY   */
} uart_clock_src_t;
...
typedef struct _uart_status {
    ...
    uint32_t tx_underflow     : 1;              /*!< Transmit data underflow detected 
                                                 (cleared on start of next send operation) */
    uint32_t rx_overflow      : 1;              /*!< Receive data overflow detected 
                                                 (cleared on start of next receive operation */
    uint32_t rx_break         : 1;              /*!< Break detected on receive 
                                                 (cleared on start of next receive operation) */
    uint32_t rx_framing_error : 1;              /*!< Framing error detected on receive 
                                                 (cleared on start of next receive operation) */
    uint32_t rx_parity_error  : 1;              /*!< Parity error detected on receive 
                                                 (cleared on start of next receive operation) */
 ...
 *                  STATUS_INVALID_REQUEST: If driver is already initialized.
 *                  STATUS_INVALID_PARAM  : Wrong parameter, for example uart_id > maximum real uart device number
 *                                          or p_config is NULL.
 */
...
* \return           STATUS_SUCCESS           If uninitialization was successful.
*                   STATUS_INVALID_PARAM     Wrong parameter, uart_id is invalid number.
*/
...
* \return           STATUS_SUCCESS           If power on/off was successful.
*                   STATUS_INVALID_PARAM     Wrong parameter, uart_id is invalid number.
*/
 ...
 * \return          STATUS_SUCCESS: If transmit was successful. For non-block mode,
 *                                  data is transmiting.
 *                  STATUS_INVALID_PARAM: Wrong parameter, like uart_id is invalid number
 *                                         length size is too large or zero.
 *                  STATUS_NO_INIT: uart device did NOT configure yet.
 *                  STATUS_EBUSY: uart device is already in previous transmitting.
 */
 ...
 * \param[in]       uart_id: identifier for uart device,
 * \return          true  If UART is transmitting.
 ...
 * \return          STATUS_SUCCESS: Receive request was successful. For non-block mode,
 *                                  data is in receiving state.
 *                  STATUS_INVALID_PARAM: Wrong parameter, like uart_id is invalid number
 *                                        length size is too large or zero.
 *                  STATUS_NO_INIT: uart device did NOT configure yet.
 *                  STATUS_EBUSY: uart device is  in previous receiving state.
 */
 ...
 * \param[in]       uart_id: identifier for uart device.
 * \return          true: If the receiver has at least one byte of data to get.
 ...
 * \return          Mask of reported errors. (if Invalid uart_id, it will get the status of all zero)
 */
 ...
 * \return          STATUS_SUCCESS: Set/Clear break request was successful.
 *                  STATUS_INVALID_PARAM: Wrong parameter, uart_id is invalid number
 *                  STATUS_INVALID_REQUEST: uart device clock should be turn on before this request.
 *                  STATUS_EBUSY: uart device is in transmitting state.
 */
 ...
 * \return          STATUS_SUCCESS           Set/Clear RTS request was successful.
 *                  STATUS_INVALID_PARAM     Wrong parameter, uart_id is invalid number
 *                  STATUS_INVALID_REQUEST   uart device clock should be turn on before this request.
 */
 ...
 * \return          data received in RX FIFO
 */
