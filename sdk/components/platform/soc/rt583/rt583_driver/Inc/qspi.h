...
#define QSPI_CLK_16M                                                           \
    (QSPI_MST_CLKDIV_EN | QSPI_MST_CLKDIV_16MHZ) /*!< SPI running 16MHz   */
#define QSPI_CLK_8M                                                            \
    (QSPI_MST_CLKDIV_EN | QSPI_MST_CLKDIV_8MHZ) /*!< SPI running  8MHz   */
#define QSPI_CLK_4M                                                            \
    (QSPI_MST_CLKDIV_EN | QSPI_MST_CLKDIV_4MHZ) /*!< SPI running  4MHz   */
#define QSPI_CLK_2M                                                            \
    (QSPI_MST_CLKDIV_EN | QSPI_MST_CLKDIV_2MHZ) /*!< SPI running  2MHz   */
#define QSPI_CLK_1M                                                            \
    (QSPI_MST_CLKDIV_EN | QSPI_MST_CLKDIV_1MHZ) /*!< SPI running  1MHz   */

#define QSPI_STATUS_TRANSFER_COMPLETE (1 << 0)
#define QSPI_STATUS_TRANSFER_SHORT    (1 << 1)

#define SPI_LSB_ORDER (0)                       /*!< SPI Least significant bit
                                                 shifted out first. */
#define SPI_MSB_ORDER (1)                       /*!< SPI Most significant bit 
                                                 shifted out first. */
...
#define SPI_CHIPSEL_ACTIVE_LOW                                                 \
    (0) /*!< Active low for Chip Select, slave selected in CS in low */
#define SPI_CHIPSEL_ACTIVE_HIGH                                                \
    (1) /*!< Active high for Chip Select,slave selected in CS in high */
...
#define QSPI_STATE_WRITE_DMA                                                   \
    8 /*!< SPI controller in DMA write (TX only) transfer state.   */
#define QSPI_STATE_READ_DMA                                                    \
    16 /*!< SPI controller in DMA write/read transfer  state.   */



/**
 * \brief           QSPI finish routine callback for user application.
 * \param[in]       qspi_id: QSPI id.  0 for QSPI0, 1 for QSPI 1
 * \param[in]       statue: QSPI transfer status.
...
typedef struct {
    ...
    uint8_t SPI_CPHA : 1;                       /*!< for SPI data sample timing in CLK low 
                                                 or high */
    uint8_t SPI_CS : 2;                         /*!< For SPI0 master mode, controller can
                                                 select 4 slave devices to send SPI signal */
    uint8_t SPI_CS_POL : 1;                     /*!< Chip select is active high or active
                                                 low in operation. SPI_CHIPSEL_ACTIVE_LOW 
                                                 or SPI_CHIPSEL_ACTIVE_HIGH    */
    uint8_t SPI_MASTER : 1;                     /*!< Select for SPI_MASTER_MODE or
                                                     SPI_SLAVE_MODE */
    uint8_t reserved : 1;
    uint16_t SPI_CLK;                           /*!< For host mode, the controller will output
                                                 CLK by this setting. For slave mode, this 
 ...
 */
typedef struct {
    uint8_t* cmd_buf;                           /*!< pointer to command buffer for
                                                 QSPI transfer, it must be 4 bytes alignment */
    uint8_t* write_buf;                         /*!< pointer to write buffer for
                                                 QSPI transfer, host will send this
                                                 buffter to spi slave device. It must 
                                                 be 4 bytes alignment */
    uint8_t* read_buf;                          /*!< pointer to read buffer for QSPI transfer,
                                                 host will receive data from slave device. 
                                                 It must be 4 bytes alignment */
    ...
    uint8_t
        select_slave_device : 2; /*!< For SPI0 master mode, controller can select 4 slave devices to send SPI signal   */
#endif
 ...
 */
typedef struct {
    uint8_t* write_buf;                         /*!< pointer to TX buffer for SPI transfer,
                                                 it could be NULL for RX only. */
    uint8_t* read_buf;                          /*!< pointer to RX buffer for SPI transfer,
                                                 it could be NULL for TX only. */
 ...
 *  to transfer.
 */
extern uint32_t qspi_init(uint32_t qspi_id,
                          const qspi_transfer_mode_t* spi_mode);
 ...
 */
extern uint32_t qspi0_set_device_polarity(uint32_t slave_id,
                                          uint32_t slave_polarity);
...
/*@}*/ /* end of RT58X_DRIVER QSPI */

#ifdef __cplusplus
}
#endif

#endif /* End of QSPI_MASTER_H */
