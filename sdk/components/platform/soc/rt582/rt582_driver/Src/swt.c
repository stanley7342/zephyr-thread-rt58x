...
static swt_evt_schedule_func_t swt_evt_schedule_func;   /* Pointer to function for propagating timeout
                                                         events to the scheduler. */
static bool swt_tick_running;                   /* Boolean indicating if Software Timer Tick is running. */
static bool swt_tick_reset;                     /* Boolean indicating if Software Timer Tick counter has
                                                 been reset due to last timer removed from timer list during
                                                 the timer list handling. */
uint32_t swt_tick_cnt = 0;                      /* Software Timer Tick counter value. */

timern_t* TIMER;
timern_t* Timer_Base[MAX_NUMBER_OF_TIMER] = {TIMER0, TIMER1, TIMER2, TIMER3,
                                             TIMER4};
...
void Timer_DeInit(uint32_t timer_id) {
    ...
    switch (timer_id) {
        case RT_TIMER0: NVIC_DisableIRQ((IRQn_Type)(Timer0_IRQn)); break;

        case RT_TIMER1: NVIC_DisableIRQ((IRQn_Type)(Timer1_IRQn)); break;

        case RT_TIMER2: NVIC_DisableIRQ((IRQn_Type)(Timer2_IRQn)); break;

        case RT_TIMER3: NVIC_DisableIRQ((IRQn_Type)(Timer3_IRQn)); break;

        case RT_TIMER4: NVIC_DisableIRQ((IRQn_Type)(Timer4_IRQn)); break;
 ...
 * \return          None
 */
void Timer_Config(uint32_t timer_id, timern_t Timer_Init,
                  timer_cb_fn timer_handler) {
    TIMER = Timer_Base[timer_id];
    ...
    if (Timer_Init.CONTROL.bit.INT_ENABLE == 1) {
        ...
        if (timer_handler != NULL) {
            timer_callback_register(timer_id, timer_handler);
        }

        switch (timer_id) {
            case RT_TIMER0:
                NVIC_SetPriority((IRQn_Type)(Timer0_IRQn),
                                 configTIMER_INTERRUPT_PRIORITY);
                NVIC_EnableIRQ((IRQn_Type)(Timer0_IRQn));
                break;

            case RT_TIMER1:
                NVIC_SetPriority((IRQn_Type)(Timer1_IRQn),
                                 configTIMER_INTERRUPT_PRIORITY);
                NVIC_EnableIRQ((IRQn_Type)(Timer1_IRQn));
                break;

            case RT_TIMER2:
                NVIC_SetPriority((IRQn_Type)(Timer2_IRQn),
                                 configTIMER_INTERRUPT_PRIORITY);
                NVIC_EnableIRQ((IRQn_Type)(Timer2_IRQn));
                break;

            case RT_TIMER3:
                NVIC_SetPriority((IRQn_Type)(Timer3_IRQn),
                                 configTIMER_INTERRUPT_PRIORITY);
                NVIC_EnableIRQ((IRQn_Type)(Timer3_IRQn));
                break;

            case RT_TIMER4:
                NVIC_SetPriority((IRQn_Type)(Timer4_IRQn),
                                 configTIMER_INTERRUPT_PRIORITY);
 ...
 */
static __INLINE uint32_t Swt_Tick_Counter_Get(void) { return swt_tick_cnt; }
 ...
 */
static __INLINE uint32_t Swt_Ticks_Diff_Get(uint32_t ticks_now,
                                            uint32_t ticks_old) {
...
static void Swt_Timeout_Handler_Exec(swt_node_t* p_node) {
    ...
    if (swt_evt_schedule_func != NULL) {
        err_code = swt_evt_schedule_func(p_node->p_timeout_handler,
                                         p_node->p_context);
    } else {
...
static bool Swt_List_Deletions_Handler(void) {
    ...
    while (queue_entry_first != p_queue->last) {
        swt_queue_entry_t* p_queue_entry =
            &p_queue->p_swt_queue_entry[queue_entry_first];
...
static void Swt_Expired_Timers_Handler(uint32_t ticks_elapsed,
    ...
    while (swt_id_head != NULL) {
        ...
        if (p_node->ticks_periodic_interval != 0) {
            p_node->ticks_at_start = (ticks_previous + ticks_expired)
                                     & MAX_SWT_TICK_COUNTER_VAL;
...
static bool Swt_List_Insertions_Handler(swt_node_t* p_restart_list_head) {
    ...
    while ((p_restart_list_head != NULL) || (p_queue->first != p_queue->last)) {
        ...
        if (p_restart_list_head != NULL) {
            ...
            swt_queue_entry_t* p_queue_entry =
                &p_queue->p_swt_queue_entry[p_queue->first];
            ...
            if ((p_queue_entry->op_type != SWT_QUEUE_ENTRY_TYPE_START)
                || p_node->is_running) {
                continue;
            }
            ...
            p_node->ticks_first_interval =
                p_queue_entry->params.start.ticks_first_interval;
            p_node->ticks_periodic_interval =
                p_queue_entry->params.start.ticks_periodic_interval;
        ...
        if (((p_node->ticks_at_start - swt_ticks_latest)
             & MAX_SWT_TICK_COUNTER_VAL)
            < (MAX_SWT_TICK_COUNTER_VAL / 2)) {
            p_node->ticks_to_expire = Swt_Ticks_Diff_Get(p_node->ticks_at_start,
                                                         swt_ticks_latest)
                                      + p_node->ticks_first_interval;
        } else {
            uint32_t delta_current_start;

            delta_current_start = Swt_Ticks_Diff_Get(swt_ticks_latest,
                                                     p_node->ticks_at_start);
            if (p_node->ticks_first_interval > delta_current_start) {
                p_node->ticks_to_expire = p_node->ticks_first_interval
                                          - delta_current_start;
            } else {
                p_node->ticks_to_expire = 0;
            }
...
static void Swt_Compare_Reg_Update(swt_node_t* p_timer_id_head_old) {
    ...
    if (swt_id_head != NULL) {
        ...
        uint32_t ticks_elapsed = Swt_Ticks_Diff_Get(pre_counter_val, cc)
                                 + SWT_TICK_COMPARE_OFFSET_MIN;
        ...
        cc += (ticks_elapsed < ticks_to_expire) ? ticks_to_expire
                                                : ticks_elapsed;
        cc &= MAX_SWT_TICK_COUNTER_VAL;

        uint32_t post_counter_val = Swt_Tick_Counter_Get();

        if ((Swt_Ticks_Diff_Get(post_counter_val, pre_counter_val)
             + SWT_TICK_COMPARE_OFFSET_MIN)
...
static void Swt_List_Handler(void) {
    ...
    if (ticks_have_elapsed) {
        Swt_Expired_Timers_Handler(ticks_elapsed, ticks_previous,
                                   &p_restart_list_head);
        compare_update = TRUE;
    }
 ...
 */
static void Swt_Queue_Entry_Enque(swt_queue_header_t* p_queue,
                                  uint8_t last_index) {
 ...
 */
static swt_queue_entry_t* Swt_Queue_Entry_Alloc(swt_queue_header_t* p_queue,
                                                uint8_t* p_last_index) {
    uint8_t last;
    swt_queue_entry_t* p_queue_entry;
...
static uint32_t Swt_Start_Node_Schedule(swt_node_t* p_node,
    ...
    swt_queue_entry_t* p_queue_entry = Swt_Queue_Entry_Alloc(p_swt_queue,
                                                             &last_index);
 ...
 * \return          STATUS_SUCCESS on successful scheduling a timer stop operation. 
 *                  STATUS_ERROR when there is no memory left to schedule the timer stop operation.
 */
 ...
 * \return          STATUS_SUCCESS on successful scheduling a timer stop all operation. 
 *                  STATUS_ERROR when there is no memory left to schedule the timer stop all operation.
 */
...
/**
 * \brief           unction for handling the software interrupt initinal
...
uint32_t Swt_Init(swt_evt_schedule_func_t evt_schedule_func) {
    ...
    p_queue_entry =
        (swt_queue_entry_t*)(&swt_queue_buffer[sizeof(swt_queue_header_t)]);
...
uint32_t Swt_Start(swt_node_t* p_node, uint32_t timeout_ticks, swt_mode_t mode,
                   swt_timeout_handler_t timeout_handler, void* p_context) {
    uint32_t timeout_periodic;
    ...
    return Swt_Start_Node_Schedule(p_node, timeout_ticks, timeout_periodic,
                                   p_context);
}
...
uint32_t Swt_Stop_All(void) { return Swt_Stop_All_Node_Schedule(); }
...
uint32_t Swt_Cnt_Diff_Compute(uint32_t ticks_to, uint32_t ticks_from,
                              uint32_t* p_ticks_diff) {
    *p_ticks_diff = Swt_Ticks_Diff_Get(ticks_to, ticks_from);
    return STATUS_SUCCESS;
}

bool Swt_Is_Running(swt_node_t* p_node) { return (p_node->is_running == TRUE); }
...
void Swt_Timeout_Handler(void) {
    ...
    if (swt_id_head != NULL) {
        ...
        ticks_elapsed = Swt_Ticks_Diff_Get(Swt_Tick_Counter_Get(),
                                           swt_ticks_latest);
