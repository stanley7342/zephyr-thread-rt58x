...
int hosal_swi_callback_register(uint32_t swi_port, uint32_t swi_trig_bit, hosal_swi_cb cb) {
	...
	swi_int_callback_register(swi_port,swi_trig_bit, cb);
...
int hosal_swi_callback_unregister(uint32_t swi_port, uint32_t swi_trig_bit) {
    ...
    NVIC_DisableIRQ((IRQn_Type)(SWI0_IRQn + swi_port));
...
int hosal_swi_set_data(uint32_t swi_port, uint32_t data) {

     if (swi_port > HOSAL_SWI_ID_MAX ) {

        return HOSAL_STATUS_INVALID_PARAM;
    }

		
    swi_set_int_data(swi_port,data);

    return HOSAL_STATUS_SUCCESS;
}

int hosal_swi_get_data(uint32_t swi_port, uint32_t* data)  {

     if (swi_port > HOSAL_SWI1_ID ) {

        return HOSAL_STATUS_INVALID_PARAM;
    }

    swi_get_int_data(swi_port,data);
